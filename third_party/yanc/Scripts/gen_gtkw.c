...
//   gen_gtkw --assembly-only <in.vcd> <out.gtkw> <trad_opcode.txt> [save.gtkw]
//     Emits only the decoded Assembly instruction track used by Solar.
//
...
static int emit_assembly_sections(const char *translation) {
    int emitted = 0;

    for (int i = 0; i < g_nproc; i++) {
        int vi = find_sig(g_proc[i].inst, "valr2");

        if (vi < 0) continue;
        emit_comment("Instructions *******");
        emit_signal(
            &g_sig[vi], FMT_DEC, COL_INDIGO, "Assembly",
            translation, NULL
        );
        emitted++;
    }
...
int main(int argc, char **argv) {
    int assembly_only = (argc == 5 || argc == 6) &&
                        strcmp(argv[1], "--assembly-only") == 0;
    if ((!assembly_only && argc < 5) ||
        (assembly_only && argc != 5 && argc != 6)) {
        fprintf(stderr,
                "usage: %s <in.vcd> <out.gtkw> <tmp_base> <comp2gtkw_exe>\n"
                "       %s --assembly-only <in.vcd> <out.gtkw> <trad_opcode.txt> [save.gtkw]\n",
                argv[0], argv[0]);
        return 1;
    }
    const char *vcd = assembly_only ? argv[2] : argv[1];
    const char *out = assembly_only ? argv[3] : argv[2];
    const char *translation = assembly_only ? argv[4] : NULL;
    const char *save = assembly_only && argc == 6 ? argv[5] : out;
    if (!assembly_only) {
        g_tmp_base = argv[3];
        g_comp2gtkw = argv[4];
    }
    ...
    if (assembly_only) {
        if (emit_assembly_sections(translation) == 0) {
            fclose(g_out);
            remove(out);
            fprintf(stderr, "gen_gtkw: VCD has no YANC Assembly instruction track\n");
            return 3;
        }
    } else {
